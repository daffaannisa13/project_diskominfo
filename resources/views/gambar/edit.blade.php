            ...
            <div class="col-xxl">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <small class="text-muted float-end">Form untuk mengedit gambar</small>
                    </div>
                    <div class="card-body">
                        <!-- Form to edit an existing image -->
                        <form action="{{ route('gambar.update', $gambar->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="url" class="form-control" id="url" name="url" placeholder="Masukkan URL gambar" value="{{ old('url', $gambar->url) }}" required />
                                ...
                                <div class="col-sm-10">
                                    <select class="form-select" id="kategori_id" name="kategori_id" required>
                                        ...
                                        @foreach($kategori as $k)
                                            <option value="{{ $k->id }}" {{ $k->id == $gambar->kategori_id ? 'selected' : '' }}>{{ $k->nama_kategori }}</option>
                                        @endforeach
                                    </select>
                                    @error('kategori_id')
                                        <small class="text-danger">{{ $message }}</small>
